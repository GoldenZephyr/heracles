...
def main(scene_graph, neo4j_uri, neo4j_creds):
    with Neo4jWrapper(neo4j_uri, neo4j_creds, atomic_queries=True, print_profiles=False) as db:
        ...
        records, summary, keys = db.execute("""MATCH (n: Object) RETURN DISTINCT n.class as class, COUNT(*) as count""")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scene_graph", type=str, default="scene_graphs/example_dsg.json")
    parser.add_argument("--neo4j_uri", type=str, default=os.getenv("HERACLES_NEO4J_URI"))
    parser.add_argument("--object_labelspace", type=str, default="")
    parser.add_argument("--room_labelspace", type=str, default="")
    args = parser.parse_args()
    assert args.neo4j_uri, "No NEO4J_URI provided -- either provide as an arg or set \"$HERACLES_NEO4J_URI\""
    ...
    scene_graph.metadata.add({"LayerIdToHeraclesLayerStr": spark_layer_id_to_heracles_layer_str})
